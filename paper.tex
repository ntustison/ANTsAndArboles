...
\title{Optimal Symmetric Multimodal Templates and Concatenated Random Forests for Supervised Brain Tumor Segmentation (made easy) with ANTsR}
...
are optimal in terms of both shape and multivariate intensity.  Further performance improvements are demonstrated by a successive application of random forest models for increasing levels of feature refinement 
 to which we refer as {\it concatenated random forests}.  Although the derived workflow involves many disparate components, the newly developed ANTsR open source software package significantly facilitates the proposed methodology.  ANTsR, a comprehensive interface of the popular Advanced Normalization Tools (ANTs) repository with the R statistical project,
...
of the National Institutes of Health and is available through our ANTsR 
package.
...
\begin{figure*}[htb]
  \centering
    \includegraphics[width=160mm]{Figures/templateKirby.pdf}
...
given $K$ image modalities, ${\mathbf I} = \{I_1,I_2,\ldots, I_K\}$, 
...
template and its symmetrical version.  This is illustrated conceptually in
...
which has demonstrated good performance in normal brain registration.
...
invertibility is essential for the features we describe below.  The first 
...
the T2 symmetric template difference image is calculated from
\begin{align}
...
as follows to create the non-Euclidean contralateral modality counterpart:
\begin{align}
...
\subsection{Other Multi-Modal Feature Image Generation}

In addition to the feature images outlined in the previous section, 
several others were generated as described as follows:
\begin{itemize}
...
we provide sample mid-axial slices from the {\tt BRATS\_HG0301}
...
set of tools for multivariate neuroimage analysis ANTsR intends to provide a modern framework for medical analytics, with a focus on imaging-assisted prediction and statistical power.
...
installation can be found on the ANTsR github website.  Feel
free to contact the authors if installation trouble occurs.  We note
that ANTsR is currently only tested on UNIX-alikes such OSX and Ubuntu
operating systems.  \footnote{Windows installation should be possible
  but, to our knowledge, has not been attempted.}.

\subsubsection{Usage}
ANTsR is intended to not only allow easy interchange between
...
An additional motivation for our development of ANTsR (and
